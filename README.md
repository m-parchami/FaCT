...
The implementation of C2-Score is quite simple. We first need the foundation model with an upsampler ([as done here](https://github.com/m-parchami/FaCT/blob/8572215c1908b9b3d881bc68d68ad388a651ba39/fact/analysis/dino_consistency.py#L189)). For a particular concept, we consider the top activating images, compute the DINO features ([as done here](https://github.com/m-parchami/FaCT/blob/8572215c1908b9b3d881bc68d68ad388a651ba39/fact/analysis/dino_consistency.py#L221)), and finally compute the per-image feature vector [with this function](https://github.com/m-parchami/FaCT/blob/8572215c1908b9b3d881bc68d68ad388a651ba39/fact/analysis/dino_consistency.py#L87). Note that we use an attribution to take a weighted average of the features (to put emphasis on where the concept is looking). If you don't have any spatial attribution or activation map for your method, you can take a uniform weighting.

Once you have a set of dino features computed for the top-activating images of your concept, we measure the pairwise cosine similarity of the feature vectors ([as done here](https://github.com/m-parchami/FaCT/blob/8572215c1908b9b3d881bc68d68ad388a651ba39/fact/analysis/dino_consistency.py#L132)). As discussed in the paper, it is important to consider a random baseline (a concept with random attribution and activaton on every image), to measure the inherent similarity of the probe dataset. For example, if you are evaluating your concept consistency on images of a single class, this will naturally increase the random baseline.
